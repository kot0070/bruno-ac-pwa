(function(root,factory){
  ...
  function normalizeBuilding(raw){raw=raw||{};var d=defaultBuilding();return {
    sqft:clamp(num(raw.sqft,d.sqft),0,100000),stories:clamp(Math.round(num(raw.stories,d.stories)),1,10),ceilingHeight:clamp(num(raw.ceilingHeight,d.ceilingHeight),6,30),
    location:text(raw.location)||d.location,foundation:text(raw.foundation)||d.foundation,ductScope:text(raw.ductScope)||d.ductScope,
    returnGrillesDesign:nullableNumber(raw.returnGrillesDesign),notes:text(raw.notes)
  }}
  function normalizeRoom(raw){raw=raw||{};var type=ROOM_TYPES.indexOf(text(raw.type))>=0?text(raw.type):'other';return {
    id:text(raw.id)||id(),type:type,count:clamp(Math.round(num(raw.count,1)),1,50),areaEach:nullableNumber(raw.areaEach),conditioned:bool(raw.conditioned,type!=='garage'),
    supplyPerRoom:nullableNumber(raw.supplyPerRoom),branchFtPerRoom:nullableNumber(raw.branchFtPerRoom),exteriorWalls:nullableNumber(raw.exteriorWalls),windows:nullableNumber(raw.windows)
  }}
  ...
  function defaultSupplyForRoom(room){
    var base=DEFAULT_SUPPLY[room.type];
    if(room.type==='living_room'&&room.areaEach!=null&&room.areaEach>300)base=2;
    if(room.areaEach!=null&&room.areaEach>500&&room.type!=='garage')base=Math.max(base,2);
    return base;
  }
  ...
  function applyOverride(metric,override){
    var base=metric.calculatedBaseline,finalValue=base,provenance=base==null?'unresolved':'calculated_baseline',blocking=[];
    if(override&&override.value!=null){
      if(metric.hardMinimum&&metric.codeMinimum!=null&&override.value<metric.codeMinimum){blocking.push({key:metric.key,type:'below_code_minimum',message:'Override '+override.value+' is below hard minimum '+metric.codeMinimum+'.'});}
      else {finalValue=override.value;provenance='override'}
    }
    return Object.assign({},metric,{override:override||{value:null,reason:'',note:''},finalQuantity:finalValue,finalSource:provenance,blocking:blocking});
  }

  function build(raw){
    var p=normalizePlan(raw),b=p.building,rooms=p.rooms,area=conditionedArea(rooms),sup=supplyBaseline(rooms),duct=ductBaseline(rooms),warnings=[],blockers=[],checks=[];
    var areaDelta=null;
    if(b.sqft>0&&area.value>0){areaDelta=round1(area.value-b.sqft);var pct=Math.abs(areaDelta)/b.sqft;if(pct>0.15)warnings.push('Room schedule area differs from building area by more than 15%; verify room areas or total square footage.');}
  ...
  return {VERSION:VERSION,REASONS:REASONS,ROOM_TYPES:ROOM_TYPES,DEFAULT_SUPPLY:DEFAULT_SUPPLY,defaultBuilding:defaultBuilding,defaultRoom:defaultRoom,defaultPlan:defaultPlan,normalizePlan:normalizePlan,build:build,calculatorInputs:calculatorInputs,preset:preset};
});
