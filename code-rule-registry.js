(function(root,factory){
  ...
  var CALCULATOR_RULE_MAP={
    ...
    'equipment-sizing':['IRC-M1401.3-EQUIPMENT-SIZING'],
    ...
    'duct-design':['ACCA-MANUAL-D-2016','LOCAL-AHJ-VERIFY'],
    'duct':['ACCA-MANUAL-D-2016','LOCAL-AHJ-VERIFY'],
    'supply-registers':['ACCA-MANUAL-D-2016','LOCAL-AHJ-VERIFY'],
    'return-grilles':['ACCA-MANUAL-D-2016','LOCAL-AHJ-VERIFY'],
  ...
  function validateLibrary(lib){
    ...
    var codeIds={};
    arr(lib.adoptedCodes).forEach(function(c,i){var id=text(c&&c.id);if(!id)errors.push('adopted_code_'+i+'_missing_id');else if(codeIds[id])errors.push('duplicate_adopted_code_'+id);else codeIds[id]=true;if(!text(c&&c.title))errors.push('adopted_code_'+i+'_missing_title');if(!/^https:\/\//.test(text(c&&c.sourceUrl)))errors.push('adopted_code_'+i+'_invalid_source_url')});
    var ruleIds={};
    arr(lib.rules).forEach(function(r,i){var id=text(r&&r.id);if(!id)errors.push('rule_'+i+'_missing_id');else if(ruleIds[id])errors.push('duplicate_rule_'+id);else ruleIds[id]=true;if(!text(r&&r.family))errors.push('rule_'+i+'_missing_family');if(!text(r&&r.section))errors.push('rule_'+i+'_missing_section');if(!text(r&&r.title))errors.push('rule_'+i+'_missing_title');if(!text(r&&r.summary))errors.push('rule_'+i+'_missing_summary');if(!/^https:\/\//.test(text(r&&r.sourceUrl)))errors.push('rule_'+i+'_invalid_source_url');if(!Array.isArray(r&&r.calculatorTags))errors.push('rule_'+i+'_missing_calculator_tags')});
    return {ok:errors.length===0,errors:errors};
  }
  function indexLibrary(lib){var rulesById={},codesById={},rulesByFamily={};arr(lib&&lib.adoptedCodes).forEach(function(c){if(c&&c.id)codesById[c.id]=c});arr(lib&&lib.rules).forEach(function(r){if(!r||!r.id)return;rulesById[r.id]=r;var fam=text(r.family);if(!rulesByFamily[fam])rulesByFamily[fam]=[];rulesByFamily[fam].push(r)});return {rulesById:rulesById,codesById:codesById,rulesByFamily:rulesByFamily}}
  ...
  return {CALCULATOR_RULE_MAP:CALCULATOR_RULE_MAP,validateLibrary:validateLibrary,indexLibrary:indexLibrary,findRule:findRule,searchRules:searchRules,normalizeRuleIds:normalizeRuleIds,resolveRuleIds:resolveRuleIds,calculatorRuleIds:calculatorRuleIds,validateCalculatorMap:validateCalculatorMap,coverage:coverage};
});
