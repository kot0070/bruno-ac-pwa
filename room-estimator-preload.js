...
try{
  ...
  var legacy=!!(snap&&snap.plan&&snap.plan.building&&Array.isArray(snap.plan.rooms));
  var plan=legacy?snap.plan:snap;
  if(!plan||!plan.building)return;
  if(legacy&&s.acCalculator){s.acCalculator.roomEstimator=plan;localStorage.setItem('bruno-ac-v1',JSON.stringify(s))}
