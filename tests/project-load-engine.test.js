...
assert(r2.sources.includes('BRUNO_TRANSPARENT_LOAD_V1'));
assert(r2.sources.includes('AUSTIN_TECHNICAL_CODES_2026'));
...
assert(rc.sources.includes('BRUNO_TRANSPARENT_LOAD_V1'));
assert(rc.sources.includes('COMMERCIAL_LOAD_AUSTIN_IECC_C403_2_1'));
assert(rc.sources.includes('AUSTIN_TECHNICAL_CODES_2026'));
assert(!rc.sources.includes('RESIDENTIAL_LOAD_MANUAL_J_01'));
let nonAustin=b(2000);nonAustin.project.jurisdiction='Hays County, TX';const rn=E.calculate(nonAustin,x(2000));
assert(rn.sources.includes('BRUNO_TRANSPARENT_LOAD_V1'));
assert(rn.sources.includes('TX_ENERGY_SINGLE_FAMILY_2015_IRC_CH11'));
assert(!rn.sources.includes('AUSTIN_TECHNICAL_CODES_2026'));
let nonAustinCommercial=b(2000);nonAustinCommercial.project.projectClass='commercial';nonAustinCommercial.project.jurisdiction='Hays County, TX';const rnc=E.calculate(nonAustinCommercial,x(2000));
assert(rnc.sources.includes('TX_ENERGY_COMMERCIAL_2015_IECC'));
assert(!rnc.sources.includes('COMMERCIAL_LOAD_AUSTIN_IECC_C403_2_1'));
assert.strictEqual(E.isAustinJurisdiction({jurisdiction:'Austin area'}),false,'Austin-area wording must not be treated as City of Austin AHJ');
assert.strictEqual(E.isAustinJurisdiction({jurisdiction:'City of Austin'}),true);
