...
function setGroup(id,preferFirst){
  var g=GROUPS.filter(function(x){return x.id===id})[0]||GROUPS[0];currentGroup=g.id;
...
function renderSubnav(g){
  ...
  var ctx=document.getElementById('phase2-context');if(ctx)ctx.textContent=g.label+' · '+(g.tabs.filter(function(t){return t[0]===tab})[0]||['',g.tabs[0][1]])[1];
}
function syncFromSource(){
  var tab=activeTab();var g=findGroupByTab(tab);currentGroup=g.id;renderGroups();renderBottom();renderSubnav(g);
}
function bind(){
  shell.addEventListener('click',function(e){var gb=e.target.closest('[data-group]');if(gb&&shell.contains(gb)){setGroup(gb.getAttribute('data-group'),false);return}var tb=e.target.closest('[data-tab]');if(tb&&shell.contains(tb)){openTab(tb.getAttribute('data-tab'))}});
...
function init(){
  ...
  currentGroup=findGroupByTab(activeTab()).id;renderGroups();renderBottom();renderSubnav(findGroupByTab(activeTab()));bind();document.body.classList.add('phase2-nav-ready');
}
