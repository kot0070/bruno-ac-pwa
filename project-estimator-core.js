...
function buildPlan(raw){
 ...
 var planningSqFtPerOutlet=type==='commercial'?600:400;
 var areaOutletAllowance=sqft>0?Math.ceil(sqft/planningSqFtPerOutlet):0;
 var supply=Math.max(0,conditionedRooms,areaOutletAllowance),returns=supply?Math.max(1,Math.ceil(supply/4)):0,checks=[],warnings=[];
 ...
 if(sqft>0){checks.push({key:'preliminary-area-allowance',level:'engineering',title:'Preliminary area-based outlet allowance',ruleId:'DUCT_DESIGN_01',sourceLabel:'Estimating heuristic only · not code minimum / not Manual D',detail:'For estimating responsiveness only, the starting supply-outlet allowance uses the greater of conditioned-room count or approximately one outlet per '+planningSqFtPerOutlet+' ft². This is not a code minimum and does not replace Manual J / Manual D airflow and duct design. Final quantity remains editable.'});}
 if(type==='residential'){
 ...
 var rows=[
  q('supply-registers','Supply outlets / registers',{calculated:supply,final:overrides['supply-registers']!=null?overrides['supply-registers']:supply,basis:'Preliminary estimating allowance = greater of conditioned-room count or area allowance (~1 per '+planningSqFtPerOutlet+' ft²). This is not a code minimum; airflow/outlet design still requires Manual J / Manual D or equivalent engineering.',ruleId:'DUCT_DESIGN_01',sourceLabel:'Duct design / AHJ',category:'Ductwork'}),
  q('return-grilles','Return grilles',{calculated:returns,final:overrides['return-grilles']!=null?overrides['return-grilles']:returns,basis:'Preliminary estimating allowance tied to the starting supply-outlet allowance. Return-air sizing and location remain design inputs, not a code minimum.',ruleId:'DUCT_DESIGN_01',sourceLabel:'Duct design / AHJ',category:'Ductwork'}),
 ...
 return {schemaVersion:3,projectType:type,sqft:sqft,systemType:systemType,indoorLocation:indoorLocation,rooms:rooms,roomArea:roomArea,conditionedRooms:conditionedRooms,planningSqFtPerOutlet:planningSqFtPerOutlet,areaOutletAllowance:areaOutletAllowance,counts:{bathrooms:bathrooms,kitchens:kitchens,garages:garages,hangars:hangars,warehouses:warehouses},quantities:rows,checks:checks,warnings:warnings,extras:extras,overrides:overrides,blocked:blocked,ready:blocked.length===0&&sqft>0&&rooms.length>0,requiresCommercialVerification:type==='commercial'};
}
